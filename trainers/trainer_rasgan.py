	...
	def train(self, num_epoch):
		...
		for epoch in range(num_epoch):
			...
			for i, data in enumerate(tqdm(self.train_dl)):
				# (1) : minimizes - log(sigmoid(D(x) - mean(D(G(z))))) - log(1 - sigmoid(D(G(z)) - mean(D(x))))
				self.netD.zero_grad()
				...
				errD = (criterion(c_xr - torch.mean(c_xf), real_label) + criterion(c_xf - torch.mean(c_xr), fake_label)) / 2.0
				errD.backward()
				...
				# (2) : minimizes -log(1-sigmoid(D(x)-mean(D(G(z))))) + -log(sigmoid(D(G(z))-mean(D(x))))
				self.netG.zero_grad()
				if(self.resample):
