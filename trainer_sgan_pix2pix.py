	...
	def train(self, num_epoch):
		...
		for epoch in range(num_epoch):
			...
			for i, (x, y) in enumerate(tqdm(self.train_dl)):
				...
				errD_real = criterion(c_xr, torch.ones(c_xr.size()).to(self.device))
				errD_fake = criterion(c_xf, torch.zeros(c_xf.size()).to(self.device))
				...
				errG_1 = criterion(c_xf, torch.ones(c_xf.size()).to(self.device))
				errG_2 = l1(fake_y, y)
				lambd = 100
