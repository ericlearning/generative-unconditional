	...
	def train(self, num_epoch):
		...
		for epoch in range(num_epoch):
			...
			for i, (x, y) in enumerate(tqdm(self.train_dl)):
				...
				c_xf = c_xf.view(-1)						# (bs)
				real_label = torch.ones(c_xr.size()).to(self.device)
				
				# calculate the discriminator loss
				errD = (torch.mean((c_xr - torch.mean(c_xf) - real_label)**2) + torch.mean((c_xf - torch.mean(c_xr) + real_label)**2)) / 2.0
				errD.backward()
				...
				errG_1 = (torch.mean((c_xf - torch.mean(c_xr) - real_label)**2) + torch.mean((c_xr - torch.mean(c_xf) + real_label)**2)) / 2.0
				errG_2 = l1(fake_y, y)
				lambd = 100
