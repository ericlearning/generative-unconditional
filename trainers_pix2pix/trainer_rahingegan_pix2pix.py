...
class Trainer_RAHINGEGAN_Pix2Pix():
	...
	def train(self, num_epoch):
		...
		for epoch in range(num_epoch):
			...
			for i, (x, y) in enumerate(tqdm(self.train_dl)):
				...
				c_xf = c_xf.view(-1)						# (bs)					# (bs)
				# calculate the Generator loss
				errG_1 = (torch.mean(torch.nn.ReLU()(1-(c_xf-torch.mean(c_xr)))) + torch.mean(torch.nn.ReLU()(1+(c_xr-torch.mean(c_xf))))) / 2.0
				errG_2 = l1(fake_y, y)
				lambd = 100
